	...
	if ((self = [super init])) {
		for (NSString *key in [JastorRuntimeHelper propertyNames:[self class]]) {
			...
			if ([value isKindOfClass:nsDictionaryClass]) {
                ...
                Class objectClass = [JastorRuntimeHelper propertyClassForPropertyName:key ofClass:[self class]];
                if (objectClass != nil && [[objectClass alloc] respondsToSelector:@selector(initWithDictionary:)]) {
                    value = [[objectClass alloc]initWithDictionary:value];
                } else {
                    NSString *type = [value objectForKey:@"@type"];
                    Class typeClass = NSClassFromString(type);//[JastorRuntimeHelper propertyClassForPropertyName:key ofClass:[self class]];
                    if (typeClass == [NSObject class]) {
                        ...
                        value = [[typeClass alloc] initWithDictionary:value];
                    }
                }
			...
			// handle array
                ...
                if ([[[klass alloc] init] respondsToSelector:@selector(initWithArray:)]) {
                    value = [[klass alloc] initWithArray:value];
                } else {
