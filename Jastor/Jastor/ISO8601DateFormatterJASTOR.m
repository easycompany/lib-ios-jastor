...
@interface ISO8601DateFormatterJASTOR(UnparsingPrivate)
...
@implementation ISO8601DateFormatterJASTOR
...
- (id) init {
	if ((self = [super init])) {
		...
		format = ISO8601DateFormatJASTORCalendar;
...
- (NSString *) stringFromDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	switch (self.format) {
		case ISO8601DateFormatJASTORCalendar:
			return [self stringFromDate:date formatString:ISO_CALENDAR_DATE_FORMAT timeZone:timeZone];
		case ISO8601DateFormatJASTORWeek:
			return [self weekDateStringForDate:date timeZone:timeZone];
		case ISO8601DateFormatJASTOROrdinal:
			return [self stringFromDate:date formatString:ISO_ORDINAL_DATE_FORMAT timeZone:timeZone];
		default:
			[NSException raise:NSInternalInconsistencyException format:@"self.format was %d, not calendar (%d), week (%d), or ordinal (%d)", self.format, ISO8601DateFormatJASTORCalendar, ISO8601DateFormatJASTORWeek, ISO8601DateFormatJASTOROrdinal];
			return nil;
	}
}
